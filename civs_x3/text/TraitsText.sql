...
--Mustang
UPDATE LocalizedText
SET Text = 'American unique Modern era air unit that replaces the Fighter.  Gains +13 attack against fighter aircraft, has +6 flight range, and gains +150% experience.'
WHERE Language = 'en_US'
...
UPDATE LocalizedText
SET Text = 'Brazilian unique Modern era unit that replaces the Battleship. Much stronger (+23 [ICON_Strength]) than the Battleship. Unlocked by Nationalism.'
WHERE Language = 'en_US'
...
UPDATE LocalizedText
SET Text = 'Unlocks the Builder ability to construct the Great Wall, unique to China.[NEWLINE][NEWLINE]Provides an +10 increase to [ICON_Strength] Defense. 3 Bonus to [ICON_Gold] Gold if adjacent to other segments. 3 additional [ICON_Culture] Culture and [ICON_Tourism] Tourism as you advance through the Technology Tree for adjacent segments. Must be built in a line along the borders of your empire.'
WHERE Language = 'en_US'
AND Tag = 'LOC_IMPROVEMENT_GREAT_WALL_DESCRIPTION';

UPDATE LocalizedText
SET Text = 'Chinese unique Medieval era unit. Ranged unit with a [ICON_Range] Range of 1 and 63 [ICON_RANGED].'
WHERE Language = 'en_US'
...
UPDATE LocalizedText
SET Text = '+23 [ICON_Strength] Combat Strength when on the same continent as the [ICON_Capital] Capital. +3 Great General points for kills.'
WHERE Language = 'en_US'
...
UPDATE LocalizedText
SET Text = 'German unique Modern era naval unit that replaces the Submarine. Cheaper to produce, +3 Sight, and +23 [ICON_Strength] Combat Strength when fighting on Ocean tiles. Able to reveal other stealthed units.'
WHERE Language = 'en_US'
AND Tag = 'LOC_UNIT_GERMAN_UBOAT_DESCRIPTION';

UPDATE LocalizedText
SET Text = '3 additional Military policy slots. +17 [ICON_Strength] Combat Strength when attacking city-states.'
WHERE Language = 'en_US'
...
UPDATE LocalizedText
SET Text = '+23 [ICON_Strength] Combat Strength if there is at least one Hoplite adjacent.'
WHERE Language = 'en_US'
AND Tag = 'LOC_ABILITY_HOPLITE_DESCRIPTION';

UPDATE LocalizedText
SET Text = 'Greek unique Ancient era unit that replaces the Spearman. +23 [ICON_Strength] Combat Strength if there is at least one adjacent Hoplite unit..'
WHERE Language = 'en_US'
...
UPDATE LocalizedText
SET Text = 'Unlocks the Builder ability to construct a Stepwell, unique to India.[NEWLINE][NEWLINE]10 [ICON_Food] Food and +3 [ICON_Housing] Housing. +3 [ICON_Faith] Faith if adjacent to a Holy Site district. +3 [ICON_Food] Food if adjacent to a Farm. Cannot be built on Hills or adjacent to another Stepwell.'
WHERE Language = 'en_US'
AND Tag = 'LOC_IMPROVEMENT_STEPWELL_DESCRIPTION';

UPDATE LocalizedText
SET Text = '-13 [ICON_Strength] Combat Strength to adjacent enemy units.'
WHERE Language = 'en_US'
AND Tag = 'LOC_ABILITY_VARU_DESCRIPTION';

UPDATE LocalizedText
SET Text = 'Expensive Indian unique Classical era unit that replaces the Horseman.  Adjacent enemy units receive -13 [ICON_STRENGTH] Combat Strength.'
WHERE Language = 'en_US'
AND Tag = 'LOC_UNIT_INDIAN_VARU_DESCRIPTION';

UPDATE LocalizedText
SET Text = '+15 [ICON_Faith] Faith for each civilization (including India) they have met that has founded a Religion and is not currently at war. Opposing civilizations receive tonfold the war weariness for fighting against Gandhi.'
WHERE Language = 'en_US'
